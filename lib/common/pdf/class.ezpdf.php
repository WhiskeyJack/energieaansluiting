...
/*
url: http://sourceforge.net/projects/pdf-php/forums/forum/261702/topic/2237585

I liked the simplicity of ezText but wanted a few more options like being able to set the x and y placement values as well as a width without having to type it out every time. Here's what I came up with, hope it helps others.

ezTextAdv($txt, [$x], [$y], [$size], [$width], [$options])
the only required argument is $txt, this is the text that you want displayed
$x is the x placement value (defaults to the left margin)
$y is the y placement value (defaults to current y value)
$size same as ezText()
$width is the total width you want the text to be (either an integer value or 'full')
$options same as ezText()

Just copy/paste the following code in the class.ezpdf.php file at the very end (right before the last } ) and enjoy!
*/


function ezTextAdv($txt, $x=null, $y=null, $size=null, $width=null, $options=array()){
  //store orig margin values to return to normal when we're done
  $default_rMargin = $this->ez['rightMargin'];
  $default_lMargin = $this->ez['leftMargin'];
  $default_tMargin = $this->ez['topMargin'];
  $default_bMargin = $this->ez['bottomMargin'];

  //set the x value (if specified) by changing the left margin
  $lMargin = (!empty($x) ? $x : $this->ez['leftMargin']);

  //set the width (if specified) by subtracting from page width
  if (!empty($width)) {
    if ($width == 'full') {
      $rMargin = $this->ez['rightMargin'];
    } else {
      $rMargin = $this->ez['pageWidth'] - ($lMargin + $width);
    }
  } else {
    $rMargin = $this->ez['rightMargin'];
  }

  //set the y value if specified
  if (!empty($y)) {
    $this->ezSetY($y);
  }

  //implement the x value and width
  $this->ezSetMargins($default_tMargin, $default_bMargin, $lMargin, $rMargin);

  //write text
  $this->ezText($txt, $size, $options);

  //reset margins to orig values
  $this->ezSetMargins($default_tMargin, $default_bMargin, $default_lMargin, $default_rMargin);
} 
